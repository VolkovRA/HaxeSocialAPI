 ...
 * Этот тип может содержать одно <b>динамическое</b> значение: (И снова привет от говнокодеров вк)
 * ad_{AD_ID} – Если приложение было открыто в результате перехода по рекламному объявлению. {AD_ID} Содержит идентификатор рекламного объявления.
 * 
 * Документация: <i>https://vk.com/dev/apps_init?f=6.%20referrer</i>
 * @author VolkovRA
 */
@:enum abstract Referrer(String) to String
{
	...
	/**
	 * Запрос.
	 * В этом случае дополнительно вернутся параметры: <code>request_key</code> и <code>request_id</code>.
	 */
	var REQUEST				= "request";
	...
	/**
	 * Название приложения в приглашении друга. (Со страницы: http://vk.com/apps)
	 */
